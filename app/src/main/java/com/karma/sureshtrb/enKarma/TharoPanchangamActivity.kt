...
val bodayana = "சதுர்தஸ்யோபரி அமாவாஸ்யா"
...
class TharoPanchangamActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val placeGeo = this.findViewById<TextView>(R.id.gEOLOCATION)
        val Varusham = this.findViewById<TextView>(R.id.year)
        val ayyanamm = this.findViewById<TextView>(R.id.ayana)
        val kalam = this.findViewById<TextView>(R.id.season)
        val rasee = this.findViewById<TextView>(R.id.rasi)
        val baksham = this.findViewById<TextView>(R.id.paksha)
        var todThithi = this.findViewById<TextView>(R.id.thithi)
        val kizhamai = this.findViewById<TextView>(R.id.day)
        val nachathirm = this.findViewById<TextView>(R.id.natchatram)
        val yog = this.findViewById<TextView>(R.id.yogam)
        val kar = this.findViewById<TextView>(R.id.karanam)
        ...
        val rb_FLiving = this.findViewById<RadioButton>(R.id.radio1G1)
        val rb_FDeceased = this.findViewById<RadioButton>(R.id.radio2G1)
        val rb_MLiving = this.findViewById<RadioButton>(R.id.radio1G2)
        val rb_MDeceased = this.findViewById<RadioButton>(R.id.radio2G2)
        val rb_MMLiving = this.findViewById<RadioButton>(R.id.radio1G3)
        val rb_MMDeceased = this.findViewById<RadioButton>(R.id.radio2G3)
        val rb_FMLiving = this.findViewById<RadioButton>(R.id.radio1G4)
        val rb_FMDeceased = this.findViewById<RadioButton>(R.id.radio2G4)
        ...
        if (rb_FLiving.isChecked ){ rg2.visibility = View.INVISIBLE }else { rg2.visibility = View.VISIBLE }
        if (rb_FDeceased.isChecked){ rg2.visibility = View.VISIBLE }else { rg2.visibility = View.INVISIBLE }
        if (rb_MLiving.isChecked || rb_MDeceased.isChecked) { rg3.visibility = View.VISIBLE }else{ rg3.visibility = View.INVISIBLE }
        if (rb_MMLiving.isChecked || rb_MMDeceased.isChecked) { rg4.visibility = View.VISIBLE}else{rg4.visibility = View.INVISIBLE}
        if (rb_FMLiving.isChecked || rb_FMDeceased.isChecked) { proceed.visibility = View.VISIBLE}else{proceed.visibility = View.INVISIBLE}
        ...
        rg1.setOnCheckedChangeListener { group, checkedId ->
            when (checkedId) {
                R.id.radio1G1 -> {
                    rb_FLiving.setTextColor(Color.BLUE)
                    rb_FLiving.setTypeface(null, Typeface.BOLD)
                    rb_FDeceased.setTextColor(Color.BLACK)
                    rb_FDeceased.setTypeface(null, Typeface.NORMAL)
                    fatherLive = "Living"
                ...
                R.id.radio2G1 -> {
                    rb_FLiving.setTextColor(Color.BLACK)
                    rb_FLiving.setTypeface(null, Typeface.NORMAL)
                    rb_FDeceased.setTextColor(Color.RED)
                    rb_FDeceased.setTypeface(null, Typeface.BOLD)
                    fatherLive = "Deceased"
                    // println("fatherLive =  $fatherLive")
                    rg2.visibility = View.VISIBLE
                    rg3.visibility = View.INVISIBLE
        ...
        rg2.setOnCheckedChangeListener { group, checkedId ->
            when (checkedId) {
                R.id.radio1G2 -> {
                    rb_MLiving.setTextColor(Color.BLUE)
                    rb_MLiving.setTypeface(null, Typeface.BOLD)
                    rb_MDeceased.setTextColor(Color.BLACK)
                    rb_MDeceased.setTypeface(null, Typeface.NORMAL)
                    motherLive = "Living"
                ...
                R.id.radio2G2 -> {
                    rb_MLiving.setTextColor(Color.BLACK)
                    rb_MLiving.setTypeface(null, Typeface.NORMAL)
                    rb_MDeceased.setTextColor(Color.RED)
                    rb_MDeceased.setTypeface(null, Typeface.BOLD)
                    motherLive = "Deceased"
                    // println("motherLive =  $motherLive")
                    rg2.isSaveEnabled
                    rg3.visibility = View.VISIBLE
        ...
        rg3.setOnCheckedChangeListener { group, checkedId ->
            when (checkedId) {
                R.id.radio1G3 -> {
                    rb_MMLiving.setTextColor(Color.BLUE)
                    rb_MMLiving.setTypeface(null, Typeface.BOLD)
                    rb_MMDeceased.setTextColor(Color.BLACK)
                    rb_MMDeceased.setTypeface(null, Typeface.NORMAL)
                ...
                R.id.radio2G3 -> {
                    rb_MMLiving.setTextColor(Color.BLACK)
                    rb_MMLiving.setTypeface(null, Typeface.NORMAL)
                    rb_MMDeceased.setTextColor(Color.RED)
                    rb_MMDeceased.setTypeface(null, Typeface.BOLD)
        ...
        rg4.setOnCheckedChangeListener { group, checkedId ->
            when (checkedId) {
                R.id.radio1G4 -> {
                    rb_FMLiving.setTextColor(Color.BLUE)
                    rb_FMLiving.setTypeface(null, Typeface.BOLD)
                    rb_FMDeceased.setTextColor(Color.BLACK)
                    rb_FMDeceased.setTypeface(null, Typeface.NORMAL)
                ...
                R.id.radio2G4 -> {
                    rb_FMLiving.setTextColor(Color.BLACK)
                    rb_FMLiving.setTypeface(null, Typeface.NORMAL)
                    rb_FMDeceased.setTextColor(Color.RED)
                    rb_FMDeceased.setTypeface(null, Typeface.BOLD)
    ...
    private fun loadData() {
        var rb_FLiving = this.findViewById<RadioButton>(R.id.radio1G1)
        var rb_FDeceased = this.findViewById<RadioButton>(R.id.radio2G1)
        var rb_MLiving = this.findViewById<RadioButton>(R.id.radio1G2)
        var rb_MDeceased = this.findViewById<RadioButton>(R.id.radio2G2)
        var rb_MMLiving = this.findViewById<RadioButton>(R.id.radio1G3)
        var rb_MMDeceased = this.findViewById<RadioButton>(R.id.radio2G3)
        var rb_FMLiving = this.findViewById<RadioButton>(R.id.radio1G4)
        var rb_FMDeceased = this.findViewById<RadioButton>(R.id.radio2G4)

        val settings = getSharedPreferences("Answer",0)
        if (settings.getBoolean("question1A", rb_FLiving.isChecked)){
            rb_FLiving.isChecked = settings.getBoolean("question1A",false)
            fatherLive = "Living"
        }
        if (settings.getBoolean("question1B", rb_FDeceased.isChecked)){
            rb_FDeceased.isChecked = settings.getBoolean("question1B", false)
            fatherLive = "Deceased"
        }
        if (settings.getBoolean("question2A", rb_MLiving.isChecked)){
            rb_MLiving.isChecked = settings.getBoolean("question2A", false)
            motherLive = "Living"
        }
        if (settings.getBoolean("question2B", rb_MDeceased.isChecked)){
            rb_MDeceased.isChecked = settings.getBoolean("question2B", false)
            motherLive = "Deceased"
        }
        if (settings.getBoolean("question3A", rb_MMLiving.isChecked)){
            rb_MMLiving.isChecked = settings.getBoolean("question3A", false)
            motherMotherLive = "Living"
        }
        if (settings.getBoolean("question3B", rb_MMDeceased.isChecked)){
            rb_MMDeceased.isChecked = settings.getBoolean("question3B", false)
            motherMotherLive = "Deceased"
        }
        if (settings.getBoolean("question4A", rb_FMLiving.isChecked)){
            rb_FMLiving.isChecked = settings.getBoolean("question4A", false)
            fatherMotherLive = "Living"
        }
        if (settings.getBoolean("question4B", rb_FMDeceased.isChecked)){
            rb_FMDeceased.isChecked = settings.getBoolean("question4B", false)
            fatherMotherLive  = "Deceased"
        }
    }
    ...
    private fun SaveRadioState() {
        ...
        editor.putBoolean("question1A", rb_FLiving.isChecked)
        editor.putBoolean("question1B", rb_FDeceased.isChecked)
        editor.putBoolean("question2A", rb_MLiving.isChecked)
        editor.putBoolean("question2B", rb_MDeceased.isChecked)
        editor.putBoolean("question3A", rb_MMLiving.isChecked)
        editor.putBoolean("question3B", rb_MMDeceased.isChecked)
        editor.putBoolean("question4A", rb_FMLiving.isChecked)
        editor.putBoolean("question4B", rb_FMDeceased.isChecked)
        editor.apply()
    }
...
//DateTimeFormatter.ofPattern("MMMM d, yyyy", Locale.ENGLISH)
        ...
        val dateTextt = sdf.format(cal.time)
        dateNow = dateTextt.toString()
                    ...
                    select("div.dpTableCardWrapper").forEachIndexed { index, element ->
                        ...
                        for (key2 in dpSunRiseRow2) {
                            if (key2.select("span.dpTitle").hasText()) {
                                ...
                                } else if (dataSunMoon2.startsWith("Moonrise")) {

                                    //mapPanch.put("MoonRise",dataSunMoon)
                                } else if (dataSunMoon2.startsWith("Moonset")) {
                                    //mapPanch.put("MoonSet",dataSunMoon)
                                }
                            }
                        }
                        var isKey2 = false
                        ...
                        for (key3 in dpSunRiseRow3) {
                            if (key3.select("span.dpTitle").hasText()) {
                                ...
                                if (dataSunMoon3.startsWith("Sunrise")) {
                                    ...
                                    //mapPanch.put("SunSet",dataSunMoon)
                                } else if (dataSunMoon3.startsWith("Moonrise")) {

                                    //mapPanch.put("MoonRise",dataSunMoon)
                                } else if (dataSunMoon3.startsWith("Moonset")) {
                                    //mapPanch.put("MoonSet",dataSunMoon)
                                }
                            }
                        }
    ...
    private fun parseWeb() = try {
        ...
        Jsoup.connect(todayWeb).get().run {
            select("div.dpPHeaderWrapper").forEachIndexed { index, element ->
                ...
                for (key1 in leftData){
                    if (key1.select("div.dpPHeaderLeftContent").hasText()) {
                        ...
                        TodayThithi = leftHeaderData.split(" ")[4]
                ...
                for (key in dpSunRiseRow) {
                    if (key.select("span.dpTitle").hasText()) {
                        ...
                        if (dataSunMoon.startsWith("Sunrise")) {
                            ...
                            sunSet = dataSunMoon
                            //mapPanch.put("SunSet",dataSunMoon)
                        } else if (dataSunMoon.startsWith("Moonrise")) {

                            //mapPanch.put("MoonRise",dataSunMoon)
                        } else if (dataSunMoon.startsWith("Moonset")) {
                            //mapPanch.put("MoonSet",dataSunMoon)
                        }
                    }
                }
            ...
            if (tamYrValue.contains("Plava")){ shakaSamvat = "ப்லவ" }
            if (tamYrValue.contains("Shubhakrit")){ shakaSamvat = "சுபகிருது" }
            if (tamYrValue.contains("Shobhana")){ shakaSamvat = "சோபகிருது" }
